                  ...
                  (ChrgBr : "DEBT") if (item.vendorJoin.custentity_payment_method.internalId[0] == "8" or item.vendorJoin.custentity_payment_method.internalId[0] == "7" or item.vendorJoin.custentity_payment_method.internalId[0] == "3") ,
                     ...
                     (    item map ((vendorItem, index) -> 
                           ...
                           (CdtrAgt :{
                               FinInstnId :{
                               	(
                               	if ((item.vendorJoin.custentity_payment_method.internalId[0] == "10" or item.vendorJoin.custentity_payment_method.internalId[0] == "4") and (!isEmpty(vendorItem.vendorJoin.custentitybankroutingswift)))
                               	ClrSysMmbId :{
                                       MmbId :  (vendorItem.vendorJoin.custentitybankroutingswift replace  "-" with "") replace " " with ""
                                       }
                                       else{
                            	(BIC : (vendorItem.vendorJoin.custentitybankroutingswift replace  "-" with "") replace " " with "")if(!isEmpty(vendorItem.vendorJoin.custentitybankroutingswift))
                                       }
                               	),
                            ...
                            Cdtr : {
                               Nm :  if (item.paidTransactionJoin == null and vendorItem.customSearchJoin.customFieldList.custrecord_flexport_bank_currency == "CNY" ) vendorItem.vendorJoin.custentity_searchable_legal_name
